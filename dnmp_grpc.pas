...
type
  ...
  public
    ...
    constructor Create(AMgr: TDnmpManager; AServiceMgr: TDnmpServiceManager; AServiceInfo: TDnmpServiceInfo); override;
    destructor Destroy(); override;
    { ќбработка команды, отправленной с указанного адреса
    JOIN <GUID_абонента> [текст]  ƒобавл€ет абонента в список подписчиков и пользователей канала.
    LEAVE <GUID_абонента> [текст]    ”бирает абонента из списка подписчиков и пользователей канала.
    SET_TOPIC <текст>    ”станавливает тему канала
    GET_TOPIC    ¬озвращает сообщение, содержащее заголовок (тему) канала.
    GET_USERS    ¬озвращает список активных подписчиков
    GET_ABONENTS    ¬озвращает список всех подписчиков
    GET_LAST_MESSAGES [число сообщений]    ¬озвращает последние сообщени€
    GET_MODE    ¬озвращает строку режимов канала
    SET_MODE    ”станавливает один или несколько режимов канала
    KICK <GUID_абонента> <причина>    ”бирает абонента из списка подписчиков с указанием причины
    BAN <GUID_абонента> <срок> <причина>    ƒобавл€ет абонента в "черный список" на заданый срок в указанием причины.
    UNBAN <GUID_абонента>    ”дал€ет абонента из "черного списка".
    GET_BANLIST    ¬озвращает "черный список" абонентов
    SAY <GUID_абонента> <текст>    —ообщение на канал от абонента
    }
    function ParseCmd(Text: string; Addr: TAddr): string; override;
    function ParseMsg(AMsg: TDnmpMsg): string; override;
  end;
